...
parser.add_argument('--mapping', default='mapping.inc')
...
with open(args.output, encoding="utf-8", mode="w+") as output:
  with open(args.mapping, encoding="utf-8", mode="w+") as mapping:
    ...
    for line in ispy_output:
        ...
        if (line.startswith(tuple(string.digits))):
            ...
            links = line.split()
            siblings = [] # list connected processors
                ...
                if link == "...":
                    ...
                    if target_processor not in linked_to and target_processor > processor:
                        ...
                        if target_processor != processor:
                            ...
                            siblings.append(target_processor)
            mapping.write(f"PROCESSOR raytrace.p[{processor}]\n")
            if siblings:
                # worker with sibling workers
                mapping.write(f"  rayTrace (forward[{processor}], back[{processor}],[")
                for i,s in enumerate(siblings):
                    if i==len(siblings)-1:
                        mapping.write(f"back[{s}]")
                    else:
                        mapping.write(f"back[{s}],")
                mapping.write ("],[")
                for i,s in enumerate(siblings):
                    if i==len(siblings)-1:
                        mapping.write(f"forward[{s}]")
                    else:
                        mapping.write(f"forward[{s}],")
                mapping.write (f"], true, {processor})\n")
            else:
                # worker with no siblings
                mapping.write(f"  rayTrace (forward[{processor}], back[{processor}],[soft[{soft}]], [soft[{soft}]], false, {processor})\n")
                soft = soft + 1
